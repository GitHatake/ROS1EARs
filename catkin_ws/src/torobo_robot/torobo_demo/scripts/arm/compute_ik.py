...
def main():
    with open("/home/aokilablinuxws/Desktop/R4_kenkyu/yuta/sotuken/csv_angle7/fk.csv") as csv_f:
        reader = csv.reader(csv_f)
        l = [row for row in reader]

    #csv_float = [[0 for v in row] for row in csv_str]
    csv_line = 3


    x = [[] for a in range(csv_line)]
    y = [[] for a in range(csv_line)]
    z = [[] for a in range(csv_line)]
    roll = [[] for a in range(csv_line)]
    pitch = [[] for a in range(csv_line)]
    yaw = [[] for a in range(csv_line)]
    ...
    for j in range(csv_line):
        ...
        res = torobo.compute_ik(x[j], y[j], z[j],np.radians(roll[j]), np.radians(pitch[j]),np.radians(yaw[j]))  #honntoha+1.775
...
class Torobo:
    ...
    def compute_fk(self, joint_angles):
        ...
        with open('/home/aokilablinuxws/2022JRM/angle_7_fk.csv','a') as f:
            writer = csv.writer(f)
    ...
    def compute_ik(self, x, y, z, roll, pitch, yaw):
        ...
        for i in range(len(ik_result.joint_state.name)):
            ...
            """
            with open('/home/aokilablinuxws/Desktop/R4_kenkyu/yuta/sotuken/csv_angle7/angle_7_ik.csv','a') as f:
                writer = csv.writer(f)
                writer.writerow([np.degrees(ik_result.joint_state.position[i][0]), np.degrees(ik_result.joint_state.position[i][1]), np.degrees(ik_result.joint_state.position[i][2]), np.degrees(ik_result.joint_state.position[i][3]), np.degrees(ik_result.joint_state.position[i][4]), np.degrees(ik_result.joint_state.position[i][5]), np.degrees(ik_result.joint_state.position[i][6])])
            """
