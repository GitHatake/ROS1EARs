...
void CsvRead(double array_data[ROW][COLUMN])
{
  ...
  std::ifstream stream("/home/tt/catkin_ws/src/arm_lecture/src/CSV/probe_p.csv");
  std::string line;
...
int main(int argc, char **argv)
{
  ...
  arm.setPlanningTime(1.0);//プランニング時間
  ...
  double current_rpy[ROW/3][3]; //エンドエフェクタの姿勢格納用
  int num;
  ...
  for (int posing_num_count = 0; posing_num_count < ROW; posing_num_count++)
  {
    ...
    if (starting_count == 0)
    {
      ...
      ret = arm.move();                                                                                                                                                                             // 指定位置・指定姿勢へ移動[asyncmove]は関数実行時ブロックしない
      ...
      //   current_rpy[0][0] = current_rpy_tmp[0];
      // }
      ros::Duration(0.13333).sleep(); // モーションキャプチャのFPSに合わせる(移動動作は0[s]と仮定...)
      starting_count++;
    }
  ...
  ofstream ofs("/home/tt/catkin_ws/src/arm_lecture/src/probe_p_result_15Hz.csv"); // ファイルパスを指定する
  ...
  //     ofs << current_joint_value[y][0] * 180 / M_PI << ", "  << current_rpy[y][1] * 180 / M_PI << ", " << current_rpy[y][2] * 180 / M_PI << ", " <<endl;
