...
from torobo_msgs.msg import ControllData, CalculateData
...
def main():
    rospy.init_node("toroboarm_arm_calculator_kaiya_node", anonymous=True)
    rospy.Subscriber(rospy.get_namespace() + 'data/controller', ControllData, callback, queue_size=1, tcp_nodelay=True)
    send = rospy.Publisher('data/calculator', CalculateData, queue_size=10)
    send_data = CalculateData()
    global wait_callback, JointValue, depth2, depth
    ...
    while not rospy.is_shutdown():
        while not rospy.is_shutdown():
            if wait_callback == True:
                ...
                break
        """
        if depth > 0.27 and depth < 0.35:
            original_position = torobo.compute_fk(joint_angles = np.radians(JointValue))
            JointValue = torobo.compute_ik(x=original_position[0], y=original_position[1], z=(original_position[2]-depth+0.25), roll=original_position[3], pitch=original_position[4], yaw=original_position[5])
            JointValue = np.degrees(JointValue)

        elif depth < 0.23 and depth > 0.1:
            original_position = torobo.compute_fk(joint_angles = np.radians(JointValue))
            JointValue = torobo.compute_ik(x=original_position[0], y=original_position[1], z=(original_position[2]-depth+0.25), roll=original_position[3], pitch=original_position[4], yaw=original_position[5])
            JointValue = np.degrees(JointValue)
            ...
            original_position = torobo.compute_fk(joint_angles = np.radians(JointValue))
            JointValue = torobo.compute_ik(x=original_position[0], y=original_position[1], z=(original_position[2]+0.05), roll=original_position[3], pitch=original_position[4], yaw=original_position[5])
            JointValue = np.degrees(JointValue)
        """
        if depth == 0.0:
            depth = 0.20
        ...
        depth = (depth-0.20)*-1
        depth2 = depth2 + (depth - depth2)*0.2

        original_position = torobo.compute_fk(joint_angles = np.radians(JointValue))
        JointValue = torobo.compute_ik(x=original_position[0], y=original_position[1], z=(original_position[2]+depth2), roll=original_position[3], pitch=original_position[4], yaw=original_position[5])
        JointValue = np.degrees(JointValue)


        print(type(JointValue))
        rospy.loginfo(JointValue)

        send_data.CalculatedJointValue = JointValue
...
def callback(recieve_data):
    global JointValue, depth, wait_callback
    JointValue = list(recieve_data.OriginalJointValue)
    depth = recieve_data.RealsenseDepth
    rospy.loginfo("!!recieved!!")
    """
    rospy.loginfo("----- calculator -----")
