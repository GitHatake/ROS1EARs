...
#import csv data
with open("/home/aokilablinuxws/catkin_ws/src/torobo_robot/torobo_demo/scripts/arm/test/test1028_fk2.csv") as csv_f:
...
for j in range(csv_line):
    ...
    csv_rpy[j] = csv_float[j][3:]   #kinematics
    #csv_rpy[j] = np.radians(csv_float[j][3:])  #matlab
...
def main():
    ...
    for i in range(csv_line):
        ...
        #w[i] = create_pose(zero_pos, [0.0, 0.0, 0.0], csv_xyz[i][0]-0.17,csv_xyz[i][1]-0.1,(csv_xyz[i][2]-1.775-0.03), csv_rpy[i][0], csv_rpy[i][1], csv_rpy[i][2])
