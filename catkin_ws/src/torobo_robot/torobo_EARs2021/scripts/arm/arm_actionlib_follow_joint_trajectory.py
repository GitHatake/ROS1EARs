...
with open('/home/aokilablinuxws/catkin_ws/src/torobo_robot/torobo_EARs2021/scripts/arm/test/test15.csv') as csv_f:
...
def main():
    ...
    try:
        ...
        follow_joint_trajectory(
            ...
            positions = np.radians(csv_float[0]), #ここにMATLABで求めたTeatcsv.csvの値を入れる
            time_from_start = 5.0
        )
        ...
        for i in range(csv_line):
            follow_joint_trajectory(
                action_client = action_client,
                joint_names = JOINT_NAMES,
                positions = np.radians(csv_float[i]), #ここにMATLABで求めたTeatcsv.csvの値を入れる
                time_from_start = 1.0
            )
